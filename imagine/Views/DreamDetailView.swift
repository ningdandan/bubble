...
//                spacer
                ...
                ScrollView {
                    VStack(spacing: 24) {
                        ...
                        Text("\(viewModel.dream.actions.count) actions")
                            .labelFont()
                            .padding(.horizontal)
                        
                        VStack(spacing: 16) {
                            ForEach(viewModel.dream.actions) { action in
...
struct DreamEditorSheetView: View {
    ...
    var body: some View {
        DreamEditorView(viewModel: editorVM, existingDreamID: existingDream.id) { updatedDream in
            onSave(updatedDream)
            dismiss()
        }
        .onAppear {
            editorVM.load(from: existingDream)
        }
    }
}
